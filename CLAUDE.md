...
- `.env` (in `~/.finance-tracker/`, not the repo) holds Plaid credentials (`PLAID_CLIENT_ID`, `PLAID_SECRET`, `PLAID_ENV`, `PORT`, `REDIRECT_URI`) — never print or commit its contents.
...
- State lives entirely in flat files under `DATA_DIR` (`.plaid_items.json`, `.plaid_history.json`, `.csv_data/*.json`, etc.) — there is no database.

**Frontend** — everything is one page (`index.html`) with all seven "sheets" (tabs) inlined as hidden/shown `<div class="sheet">` blocks, switched via `showTab()`. All behavior lives in the single `script.js` (~3300 lines, no modules/bundler); `styles.css` holds all styling.
