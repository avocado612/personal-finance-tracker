...
app.get('/api/transactions', async (req, res) => {
    ...
    try {
        ...
        const EXCLUDE_PLAID_CATS = new Set([
            ...
            // as TRANSFER_IN by Plaid and must come through. Real inter-account transfers
            // are caught by TRANSFER_PATTERNS below.
        ]);
        ...
        // Log any June 2026 transactions found (for debugging)
        [...txnMap.values()]
            .filter(t => t.date && t.date.startsWith('2026-06'))
            .forEach(t => console.log(`  June txn: ${t.date} | ${t.name} | $${t.amount} | cat:${t.personal_finance_category?.primary} | acct:${acctMap.get(t.account_id)?.name}`));
...
// After WF OAuth, Plaid redirects back here with ?oauth_state_id=...
// The frontend Plaid.create() handler picks it up automatically when re-initialized
// with the same link_token; this route just serves index.html so the page loads.
app.get('/', (req, res) => {
...
if (hasCerts) {
    ...
    // Fallback: plain HTTP (sandbox only)
    app.listen(PORT, () => {
